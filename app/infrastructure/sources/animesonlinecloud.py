...
class AnimesOnlineCloud(AnimeSource):
    ...
    def search_by(self, name: str) -> list[Anime]:
        ...
        if not q:
            return []
        response = requests.get(
            f"{self.base_url}/?s={quote(q, safe='')}&post_type=animes",
            headers=HEADERS,
            timeout=20,
        )
        if not validate_response(response):
        ...
        # DooPlay result-item / articles (evita widgets laterais)
        ...
        for article in items:
            # título real (evita pegar o <a> do poster com texto "TV")
            ...
            retrieved.append(
                Anime(title=raw_title, rating=rating, link=link, image=image)
            )
        return retrieved
    ...
    def get_anime_details(self, link: str) -> Anime:
        ...
        # breadcrumb lixo: "HomeAnimesTitle"
        ...
        for i, block in enumerate(season_blocks):
            ...
            for a in block.select("a[href*='/episodio']"):
                ...
                episodes.append(
                    Episode(number=ep_num, title=text, link=href, video_src="")
                )
            if episodes:
