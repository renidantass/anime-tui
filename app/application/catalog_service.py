...
class CatalogService:
    def __init__(
        self,
        external_api: IAniListClient | None,
        genre_labels: dict,
        sd: ISourceDiscovery,
        get_enabled_list,
        catalog_resolver,
    ...
    def get_genres(self) -> list[dict[str, str]]:
        ...
        try:
            ...
            return []
        return [
            {"id": name, "name": name, "label": self._genre_labels.get(name, name)}
            for name in names
        ]
    ...
    def catalog_by_genre(self, genre: str, *, page: int = 1, per_page: int = 24) -> dict:
        ...
        if not genre or not self._has_api():
            return empty
        try:
            al_page = self._api.get_anime_by_genre(
    ...
    def get_meta(self, *, title: str = "", external_id: int | None = None) -> dict | None:
        ...
        data["relations"] = [
            ...
            if any(f.get("id") == r.get("id") and f.get("available") for f in data["franchise"])
    ...
    def _annotate_airing(self, items: list[dict]) -> list[dict]:
        ...
        if detail_jobs:
            ...
            futures = {
                pool.submit(
                    _episode_map_for_source_link, self._sd, self._get_enabled_list, name, link
                ): key
                ...
                matching.append(
                    {
                        ...
                        or detect_audio_variant(getattr(src, "title", "") or "", src.link or ""),
                        "title": getattr(src, "title", "") or "",
                    }
                )
            if matching:
                row["available"] = True
                row["sources"] = matching
            else:
                row["available"] = False
                row["sources"] = []
                row["unavailable_reason"] = "anime_only" if entry.sources else "missing"
            out.append(row)
        return out
    ...
    def _filter_franchise(self, franchise: list[dict]) -> list[dict]:
        if not franchise:
            return []
        current = [
            f for f in franchise if f.get("is_current") or f.get("relation_type") == "CURRENT"
        ]
        others = [
            f for f in franchise if not (f.get("is_current") or f.get("relation_type") == "CURRENT")
        ]
        candidates = []
        for f in others:
...
def _episode_map_for_source_link(
    sd: ISourceDiscovery, get_enabled_list, source_name: str, anime_link: str
) -> dict[str, dict]:
