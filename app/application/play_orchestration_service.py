"""Orquestração completa de play: resolve stream + grava histórico + busca progresso.

Combina StreamResolutionService + AnimeService + WatchHistoryService +
StreamSessionStore em um único ponto de entrada para ambas as UIs.
"""
...
from app.infrastructure.sessions.stream_session_store import (
    StreamSession,
    StreamSessionStore,
)
...
class PlayRequest:
    """Entrada para o PlayOrchestrationService."""
...
class PlayOrchestrationService:
    """Orquestra todo o fluxo de play: resolve stream, grava histórico, busca progresso."""
    ...
    def __init__(
        ...
        self._sessions = session_store or StreamSessionStore()

    def play(self, req: PlayRequest) -> PlayResult:
        """Executa o fluxo completo de play.

        Args:
            req: PlayRequest com candidatos e metadados.

        Returns:
            PlayResult com info de stream, token, progresso, etc.
        """
    ...
    def _resolve(self, candidates: list[PlayCandidate]) -> ResolvedPlay | None:
        """Resolve stream com fallback entre fontes candidatas."""
