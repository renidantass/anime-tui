"""Resolução de stream para a interface web (reusa lógica do player TUI)."""
...
@dataclass(slots=True)
class ResolvedPlay:
    """Resultado de tentar uma ou mais fontes."""
...
def finalize_play_context(ctx: PlayContext) -> PlayContext:
...
def build_upstream_headers(ctx: PlayContext) -> dict[str, str]:
...
def probe_stream(url: str, headers: dict[str, str] | None = None) -> tuple[bool, str]:
    ...
    try:
        with requests.get(
            ...
            # alguns CDNs devolvem octet-stream / binary sem video/*
            ok_ct = (
                not ct
            ...
            # ler um pedaço para garantir que o body existe
...
def order_candidates(
    *,
    ...
    # fallback legado: só episode_link
...
def resolve_with_fallback(
    *,
    ...
    for cand in candidates:
        ...
        if require_probe:
            ok, reason = probe_stream(url, headers)
            if not ok:
    ...
    if tried:
        # última tentativa com algum contexto? não — nada útil
        return ResolvedPlay(
